...
import { AtSign, Check, Loader2, Send, Square, Trash2 } from 'lucide-react'
...
import { AgentWorkbench } from './workbench/AgentWorkbench'
...
function toStoreApi(api: ChatPanelApi): ChatStoreApi {
  return {
    ...
    ...(api.denyAgentTool ? { denyAgentTool: api.denyAgentTool } : {}),
...
export function ChatPanel({ api = window.comicCanvas, onApplyPlan, workflowId = 'default' }: ChatPanelProps): JSX.Element {
  ...
  const activeRunView = useStore(handle.store, (state) => state.activeRunView)
  ...
  const headerActions = (
    <>
      <button
        ...
        className="flex h-8 w-8 items-center justify-center rounded-md text-text-muted transition-colors hover:bg-bg-hover hover:text-text-base disabled:cursor-not-allowed disabled:opacity-35"
      >
        <Trash2 className="h-3.5 w-3.5" aria-hidden="true" />
      </button>
      <label className="inline-flex cursor-pointer items-center gap-2 rounded-md px-1.5 text-[11px] text-text-secondary">
        <input
          type="checkbox"
          role="switch"
  ...
  const composer = (
    <div className="p-4">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center gap-1.5 rounded-md border border-border-input bg-bg-input px-2 py-1 text-[11px] font-medium text-text-secondary">
      ...
      <div className="relative">
        ...
        <textarea
          ...
          rows={3}
          placeholder="描述你要生成的漫剧内容，或直接提问…"
          className="min-h-[82px] w-full resize-none bg-transparent text-[14px] leading-relaxed text-text-base outline-none placeholder:text-text-muted"
        />
      </div>
      <div className="mt-3 flex items-center justify-between gap-3">
        <p className="m-0 text-[11px] text-text-muted">Enter 发送 · Shift+Enter 换行</p>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-label="停止 Agent"
            onClick={stopRun}
            className="inline-flex h-9 items-center gap-2 rounded-md border border-border-secondary px-3 text-[12px] font-semibold text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-base"
          >
          ...
          <button
            ...
            onClick={sendMessage}
            className={cn(
              'inline-flex h-9 items-center gap-2 rounded-md bg-brand px-3 text-[12px] font-semibold text-bg-base',

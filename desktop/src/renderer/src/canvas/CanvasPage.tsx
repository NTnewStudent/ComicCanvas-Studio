...
import { buildGenerationTaskStatusList, terminalFailureToNodePatch, terminalResultToNodePatch } from './lib/job-reconciliation'
...
const NODE_OPTION_LABEL_OVERRIDES: Partial<Record<NodeType, string>> = {
  imageConfigV2: 'Image Generation',
  videoConfigV2: 'Video Generation',
  videoCompose: 'Video Compose',
  superResolution: 'Super Resolution',
  muxAudioVideo: 'Mux Audio Video',
}

const MIGRATED_NODE_MENU_GUARD: ReadonlyArray<{ type: NodeType }> = [
  { type: 'character' },
  { type: 'scene' },
  { type: 'audio' },
  { type: 'videoCompose' },
...
const ADDABLE_NODE_OPTIONS = getAddableNodeDefinitions().map((definition) => ({
  ...
  label: NODE_OPTION_LABEL_OVERRIDES[definition.type] ?? definition.label,
      ...
      <div className="relative flex-1" onDragOver={handleCanvasDragOver} onDrop={(event) => void handleCanvasDrop(event)}>
        ...
        >
          <button
            ...
            title={isAddMenuOpen ? '收起' : 'Add node'}

...
function findCompletedEvent(window, jobId) {
...
function completedAssetId(window, jobId) {
...
function expectedStubImageHash(input) {
...
describe('main process runtime wiring', () => {
    it('registers product IPC handlers and drains agent/runNode jobs through local services', async () => {
        ...
        try {
            ...
            const toolList = await handlers.get('tool.list')?.({}, {});
    ...
    it('uses the built-in comic-drama planner through chat IPC when no external planner is injected', async () => {
        ...
        try {
            ...
            await runtime.waitForIdleForTests();
            const plan = await handlers.get('canvas.chatGetPlan')?.({}, { messageId: 'message-comic' });
    ...
    it('passes persisted project styles into runNode generation jobs', async () => {
        ...
        try {
            ...
            const baselineTicket = await handlers.get('canvas.runNode')?.({}, { nodeId: 'image-1' });
            await runtime.waitForIdleForTests();
            ...
            const styledTicket = await handlers.get('canvas.runNode')?.({}, { nodeId: 'image-1' });
            await runtime.waitForIdleForTests();
    ...
    it('exposes persisted runNode job state through job.get before and after execution', async () => {
        ...
        try {
            ...
            await handlers.get('canvas.saveGraph')?.({}, { projectId: 'default', graph });
            const ticket = await handlers.get('canvas.runNode')?.({}, { nodeId: 'image-observable' });
            const queued = await handlers.get('job.get')?.({}, { jobId: ticket.jobId });
            ...
            await runtime.waitForIdleForTests();
            const completed = await handlers.get('job.get')?.({}, { jobId: ticket.jobId });
    ...
    it('exposes persisted runNode jobs through filtered job.list IPC', async () => {
        ...
        try {
            ...
            await handlers.get('canvas.saveGraph')?.({}, { projectId: 'default', graph });
            const first = await handlers.get('canvas.runNode')?.({}, { nodeId: 'image-list-a' });
            const second = await handlers.get('canvas.runNode')?.({}, { nodeId: 'image-list-b' });
            const latestImages = await handlers.get('job.list')?.({}, { type: 'canvas.generateImage', limit: 2 });
            const targetOnly = await handlers.get('job.list')?.({}, { targetId: 'image-list-a' });
    ...
    it('uses the requested workflow graph and default style when runNode includes workflowId', async () => {
        ...
        try {
            ...
            const workflow = await handlers.get('canvas.createWorkflow')?.({}, { name: 'Styled episode' });
            const graph = {
                nodes: [
            ...
            const ticket = await handlers.get('canvas.runNode')?.({}, { workflowId: workflow.id, nodeId: 'image-workflow' });
            await runtime.waitForIdleForTests();
    ...
    it('drains text polish runNode jobs as terminal text results', async () => {
        ...
        try {
            ...
            const ticket = await handlers.get('canvas.runNode')?.({}, { nodeId: 'text-polish-runtime' });
            await runtime.waitForIdleForTests();

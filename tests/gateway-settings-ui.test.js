...
import { GatewayList } from '../desktop/src/renderer/src/settings/GatewayList';
...
function createApi(overrides = {}) {
    return {
        ...
        saveGateway: vi.fn().mockImplementation((input) => Promise.resolve({
            id: input.id ?? 'gw-new',
            name: input.name,
            type: input.type,
...
describe('M3 Gateway settings UI', () => {
    it('renders gateway cards with masked keys, model mapping, and enabled state', async () => {
        render(_jsx(GatewayList, { api: createApi() }));
    ...
    it('saves a new gateway with API key, capabilities, and model mappings', async () => {
        ...
        render(_jsx(GatewayList, { api: api }));
    ...
    it('submits existing key refs when editing without a new secret', () => {
        ...
        render(_jsx(GatewayForm, { gateway: existingGateways[0], onSubmit: onSubmit, onCancel: vi.fn() }));
    ...
    it('fetches OpenAI-compatible models and drag-routes one into a channel', async () => {
        ...
        render(_jsx(GatewayForm, { onSubmit: onSubmit, onCancel: vi.fn(), fetchModels: fetchModels }));
        ...
        fireEvent.drop(screen.getByRole('textbox', { name: '视频模型键' }).previousElementSibling, { dataTransfer });
        fireEvent.click(screen.getByRole('button', { name: '保存网关' }));

...
import { connectCanvasNodes, defaultCanvasNodeData, deleteCanvasNode, duplicateCanvasNode, } from '../shared/canvas-actions';
...
const actor = { type: 'agent', id: 'canvas-action-semantics' };
...
describe('Task 54 shared canvas action semantics', () => {
    ...
    it('uses identical connect, duplicate, and delete semantics for UI store and ToolRuntime', async () => {
        ...
        const runtime = createToolRuntime({
            ...
            tools: createCanvasTools({
                ...
                idFactory: (() => {
                    ...
                    return (prefix) => ids.shift() ?? `${prefix}-fallback`;
                })(),
                clock: () => 100,
            }),
        });
        ...
        if (!connected.ok)
            throw new Error('expected shared connection to succeed');

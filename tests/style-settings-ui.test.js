...
import { StyleLibrary } from '../desktop/src/renderer/src/settings/StyleLibrary';
const inkStyle = {
    id: 'style-ink',
    code: 'ink',
    name: 'Industrial Ink',
...
function createApi(overrides = {}) {
    return {
        ...
        saveStyle: vi.fn().mockImplementation((input) => Promise.resolve({
            ...inkStyle,
...
describe('REQ-094 style library UI', () => {
    it('uses Chinese production copy for style management controls', async () => {
        render(_jsx(StyleLibrary, { api: createApi() }));
    ...
    it('creates and deletes styles through typed preload actions', async () => {
        const saveStyle = vi.fn().mockImplementation((input) => Promise.resolve({
            ...inkStyle,
            id: input.id ?? 'style-neon',
            code: input.code,
            name: input.name,
        ...
        render(_jsx(StyleLibrary, { api: api }));

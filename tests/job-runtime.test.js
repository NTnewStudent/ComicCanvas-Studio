...
describe('M1 JobRuntime skeleton', () => {
    it('enqueues a durable pending job and returns only a ticket', () => withRuntimeFixture(({ jobs }) => {
    ...
    it('moves a pending job through processing to completed and emits one completed event', async () => withRuntimeFixture(async ({ jobs }) => {
    ...
    it('emits completed asset hooks for report results with asset metadata', async () => withRuntimeFixture(async ({ jobs }) => {
        const events = createJobEventBus();
        const completedReferences = [];
        const queue = createJobQueue({
            jobs,
    ...
    it('moves a pending job through processing to failed and emits one failed event', async () => withRuntimeFixture(async ({ jobs }) => {
        const events = createJobEventBus();
        const queue = createJobQueue({
            jobs,
    ...
    it('preserves structured handler error classes for agent/runtime failures', async () => withRuntimeFixture(async ({ jobs }) => {
        const events = createJobEventBus();
        const queue = createJobQueue({
            jobs,
    ...
    it('requeues abandoned processing jobs during startup recovery', () => withRuntimeFixture(({ jobs }) => {
        jobs.create({
            id: 'job-stale-1',
            type: 'canvas.generateImage',
            status: 'processing',
    ...
    it('lists durable jobs by status, type, target, and bounded newest-first limit', () => withRuntimeFixture(({ jobs }) => {
        jobs.create({
            id: 'job-list-old',
            type: 'canvas.generateImage',
            status: 'pending',

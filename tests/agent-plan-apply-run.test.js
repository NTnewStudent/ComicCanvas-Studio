...
import { AGENT_PLAN_AUTO_APPLY_ENABLED, isAgentPlanAutoApplyEnabled, resolveAgentPlanAutoApplyOptions, } from '../shared/agent-plan-apply';
...
describe('Task 60 agent plan apply/run', () => {
    ...
    it('runs image and video plan steps serially through the execution controller', () => {
        ...
        const launched = [];
        const runNode = vi.fn((nodeId) => {
            launched.push(nodeId);
            return { jobId: `job-${nodeId}`, status: 'pending', createdAt: 1_783_800_000_000 };
        });
    ...
    it('short-circuits remaining run steps after a failed video step', () => {
        ...
        const runNode = vi.fn((nodeId) => ({ jobId: `job-${nodeId}`, status: 'pending', createdAt: 1_783_800_000_111 }));
        const controller = createCanvasPlanExecutionController({
            store,
            runNode,
            applyOptions: { idFactory: (ref) => `plan-node-${ref}` },

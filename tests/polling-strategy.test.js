...
describe('M3 async media polling strategy', () => {
    it('returns completed results after pending polls with exponential backoff', async () => {
        ...
        const sleeps = [];
        const states = [
            { state: 'pending', progress: 25, message: 'queued' },
            { state: 'pending', progress: 50, message: 'rendering' },
            { state: 'completed', result: 'asset-ready' }
        ];
        const progress = [];
    ...
    it('throws provider_timeout when timeout elapses during backoff', async () => {
        let now = 0;
        const sleeps = [];
        await expect(pollWithBackoff(() => Promise.resolve({ state: 'pending', progress: 10 }), {
            initialDelayMs: 100,
            maxDelayMs: 500,
            timeoutMs: 250,
            clock: () => now,
    ...
    it('normalizes failed poll states to provider_request_failed', async () => {
        await expect(pollWithBackoff(() => Promise.resolve({ state: 'failed', message: 'remote quota exceeded', retryable: false }), {
            initialDelayMs: 100,
            maxDelayMs: 500,
            timeoutMs: 1_000
        })).rejects.toMatchObject({

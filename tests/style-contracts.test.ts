...
import type { IpcRequestMap, IpcResponseMap, StyleIpcChannel } from '../shared/ipc'

const projectStyle: StylePresetView = {
  id: 'style-project',
  code: 'ink-noir',
  name: '水墨黑白',
  description: '高反差水墨漫画',
...
const nodeStyle: StylePresetView = {
  ...projectStyle,
  id: 'style-node',
  code: 'pastel',
  name: '柔和彩铅',
...
describe('style preset shared contracts', () => {
  ...
  it('registers typed style IPC channels without implying runtime handler completion', () => {
    const channels: StyleIpcChannel[] = [
      'style.list',
      'style.save',
      'style.delete',
      'style.setProjectDefault',
    ...
    type _ListRequest = IpcRequestMap['style.list']
    type _ListResponse = IpcResponseMap['style.list']
    type _SaveRequest = IpcRequestMap['style.save']
    type _SetDefaultRequest = IpcRequestMap['style.setProjectDefault']
    type _GetDefaultRequest = IpcRequestMap['style.getProjectDefault']
    type _GetDefaultResponse = IpcResponseMap['style.getProjectDefault']

    const listRequest: _ListRequest = { includeDisabled: true }
    const listResponse: _ListResponse = [projectStyle]
    const saveRequest: _SaveRequest = {
      code: 'ink-noir',
      name: '水墨黑白',
      promptBefore: '水墨漫画',
      promptAfter: '宣纸肌理',
    }
    const defaultRequest: _SetDefaultRequest = {
      workflowId: 'workflow-1',
      stylePresetId: 'style-project',
    }
    const getDefaultRequest: _GetDefaultRequest = { workflowId: 'workflow-1' }
    const getDefaultResponse: _GetDefaultResponse = {
      workflowId: 'workflow-1',
      stylePresetId: 'style-project',
    }
  ...
  it('injects promptBefore and promptAfter around trimmed content with newline separators', () => {
    expect(composeStyledPrompt('  一只猫站在雨夜屋顶  ', projectStyle)).toBe(
      '水墨漫画，黑白高反差\n一只猫站在雨夜屋顶\n宣纸肌理，电影构图',
    )
  })

  it('skips empty prompt parts and falls back to legacy prompt preset only when before and after are empty', () => {
    expect(composeStyledPrompt('一只猫', { ...projectStyle, promptBefore: '  ', promptAfter: '柔光' })).toBe(
      '一只猫\n柔光',
    )

...
function runtimeFor(): ReturnType<typeof createToolRuntime> {
...
describe('universal read-only fs tools', () => {
  ...
  it('reads a file within the workspace and reports a workspace-relative path', async () => {
    ...
    expect((result.output as { content: string }).content).toContain('hello')
  })
  ...
  it('globs files by pattern', async () => {
    ...
    expect((result.output as { matches: string[] }).matches).toEqual(['src/a.ts', 'src/b.ts'])
  })
  ...
  it('greps file contents and returns matching lines', async () => {
    ...
    const matches = (result.output as { matches: Array<{ path: string; line: number; text: string }> }).matches

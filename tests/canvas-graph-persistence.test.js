...
function expectWorkflowImportResult(value) {
...
describe('M2 canvas graph save/load IPC', () => {
    it('persists the latest graph version, survives handler recreation, and drops invalid edges', async () => {
        ...
        try {
            ...
            const workflowRow = db.prepare('SELECT updated_at FROM workflows WHERE id = ?').get('project-1');
    ...
    it('round trips migrated hjwall node data and removes unknown node types with their edges', async () => {
        ...
        try {
            ...
            const loaded = await ipc.handlers.get('canvas.loadGraph')?.({}, { projectId: 'project-migrated' });
    ...
    it('exports sanitized workflow JSON and imports it as a new workflow without absolute asset paths', async () => {
        ...
        try {
            ...
            const exported = await ipc.handlers.get('canvas.exportWorkflow')?.({}, { workflowId: 'project-export' });
            ...
            const loaded = await ipc.handlers.get('canvas.loadGraph')?.({}, { projectId: imported.workflowId });
    ...
    it('lists workflow versions with debug metadata and restores one as a new latest graph version', async () => {
        ...
        try {
            ...
            const loaded = await ipc.handlers.get('canvas.loadGraph')?.({}, { projectId: 'project-restore' });

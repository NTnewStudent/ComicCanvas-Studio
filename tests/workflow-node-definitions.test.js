...
import { getAddableNodeDefinitions, getConnectCreateNodeDefinitions, getNodeDefinition, getRunnableNodeDefinitions, getWorkflowNodeDefinitions, } from '../shared/workflow-node-definitions';
describe('Task 42 workflow node definitions service', () => {
    it('covers every shared node type with capabilities and input/output metadata', () => {
        ...
        for (const definition of definitions) {
            ...
            expect(definition.allowedInputs).toEqual(Object.entries(NODE_CONNECTION_MATRIX)
    ...
    it('maps runnable node types to explicit run actions for future tools and agents', () => {
        const runActions = new Map(getWorkflowNodeDefinitions().map((definition) => [definition.type, definition.runAction]));

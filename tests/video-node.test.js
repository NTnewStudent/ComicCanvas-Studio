...
import { VideoNode } from '../desktop/src/renderer/src/canvas/nodes/VideoNode';
const defaultData = {
    label: 'Video 1',
    promptOverride: '',
    modelId: 'stub-video',
...
function renderVideoNode(overrides = {}) {
    const onChange = vi.fn();
    const data = { ...defaultData, ...overrides.data };
    render(_jsx(ReactFlowProvider, { children: _jsx(VideoNode, { ...overrides, id: overrides.id ?? 'video-1', data: data, assetOptions: overrides.assetOptions ?? [

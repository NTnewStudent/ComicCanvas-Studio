...
import { AssetPanel } from '../desktop/src/renderer/src/assets/AssetPanel';
const rootFolder = {
    id: 'folder-scenes',
    name: 'Scenes',
    parentId: null,
...
function makeAsset(id, mediaType, createdAt, categoryIds) {
    return {
        id,
        mediaType,
        status: 'ready',
...
function renderAssetPanel(api, initialEntry = '/assets') {
    render(_jsx(MemoryRouter, { initialEntries: [initialEntry], children: _jsx(AssetPanel, { api: api }) }));
}
function LocationProbe() {
    const location = useLocation();
    return _jsx("output", { "aria-label": "\u5F53\u524D\u4F4D\u7F6E", children: `${location.pathname}${location.search}` });
}
function renderAssetPanelWithLocation(api, initialEntry = '/assets') {
    render(_jsxs(MemoryRouter, { initialEntries: [initialEntry], children: [_jsx(AssetPanel, { api: api }), _jsx(LocationProbe, {})] }));
}
afterEach(() => {
    vi.restoreAllMocks();
    cleanup();
...
describe('M5 AssetPanel folder UI', () => {
    it('renders nested folders, creates child folders, moves assets, and deletes the selected folder', async () => {
        ...
        await waitFor(() => expect(mockOf(api.createAssetFolder)).toHaveBeenCalledWith(expect.objectContaining({ name: 'References', parentId: 'folder-shots', type: 'mixed' })));
    ...
    it('filters image assets by category and creates user-defined categories', async () => {
        ...
        await waitFor(() => expect(mockOf(api.createAssetCategory)).toHaveBeenCalledWith(expect.objectContaining({ name: '主角图' })));
    ...
    it('imports selected files with the active folder and category context', async () => {
        ...
        await waitFor(() => expect(mockOf(api.importAsset)).toHaveBeenCalledWith({
    ...
    it('renders loading, error, and empty states for the asset page shell', async () => {
        ...
        const { unmount } = render(_jsx(MemoryRouter, { initialEntries: ['/assets'], children: _jsx(AssetPanel, { api: loadingApi }) }));
    ...
    it('renames an asset from the preview modal without changing the asset URL', async () => {
        ...
        await waitFor(() => expect(mockOf(api.renameAsset)).toHaveBeenCalledWith({ assetId: 'asset-frame', displayName: '主角定稿' }));
    ...
    it('edits and disables custom image categories from the sidebar', async () => {
        ...
        await waitFor(() => expect(mockOf(api.updateAssetCategory)).toHaveBeenCalledWith({

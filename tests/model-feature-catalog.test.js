...
import { buildModelCatalog, filterWorkflowNodeDefinitions, getAddableNodeDefinitions, getConnectCreateNodeDefinitions, getRunnableNodeDefinitions, } from '../shared/workflow-node-definitions';
const gateways = [
    {
        id: 'gw-main',
        name: 'Main Gateway',
...
describe('Task 51 model and feature catalog', () => {
    ...
    it('filters disabled node definitions for add, connect-create, and run surfaces', () => {
        ...
        expect(getAddableNodeDefinitions({ featureFlags }).map((definition) => definition.type)).not.toEqual(expect.arrayContaining(['videoConfigV2', 'superResolution', 'mjImage']));
        expect(getConnectCreateNodeDefinitions('video', { featureFlags }).map((definition) => definition.type)).not.toEqual(expect.arrayContaining(['videoConfigV2', 'superResolution', 'mjImage']));
        expect(getRunnableNodeDefinitions({ featureFlags }).map((definition) => definition.type)).not.toEqual(expect.arrayContaining(['videoConfigV2', 'superResolution', 'mjImage']));

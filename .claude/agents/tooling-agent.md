...
AsyncGenerator 主循环（参考 cc-haha `query()`）：`while(true)` + state 计算 + `continue`，无递归。流式调用模型，流式中即可执行工具。只读工具并行（≤8），写入工具串行。
...
### IPC（main/ipc/）
通道名格式 `domain.action`（如 `canvas.runNode`、`job.subscribe`）。所有 `ipcMain.handle` 入参 Zod 校验。handler 薄（校验+转调）、service 厚。渲染层只能用 preload contextBridge 暴露的白名单 API。
